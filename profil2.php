...
<body>
  ...
  <div class="main">
    ...
    <form method="POST">
        ...
        <p><input type="text" name="Profession" id ="Profession" placeholder="Profession"  required></p>
        <p><input type="text" name="Project" id="Project" placeholder="Project"  required></p>
        <p><input type="text" name="competance" id="competance" placeholder="competance"  required></p>
        <p><input type="text" name="resultat" id="resultat" placeholder="resultat"  required></p>
        <p><input type="submit" name="formprof" id="formprof" value="Submit"  required></p>
...
<?php
    if(isset(($_POST["formprof"]))){
        extract($_POST);
        if(!empty($Phone)&& !empty($Profession) && !empty($Project)&& !empty($competance)&& !empty($resultat)){

            include 'config.php';
            global $db;
                $q=$db->prepare("SELECT*FROM Profil WHERE Email=:Email");
                $q->execute([
                ...
                if(!isset($result['Phone'])){
                    $q = $db->prepare("INSERT INTO profil(Email,Phone,Profession,Project,competance,resultat)VALUES(:Email,:Phone,:Profession,:Project,:competance,:resultat)");
                    $q->execute([
                    'Email'=>$_SESSION['Email'],
                    'Phone'=>$Phone,
                ...
                echo "<script>location.href='/phpmyadmin/hackaton/profil.php';</script>";

                }
                else{
                    $n=$db->prepare("UPDATE profil SET Phone=:Phone,Profession=:Profession,competance=:competance,resultat=:resultat,Project=:Project WHERE Email=:Email");
                    $n->execute([
                    'Email'=>$_SESSION['Email'],
                    'Phone'=>$Phone,
                    'Profession'=>$Profession,
            ...
            echo "completez tous les champs ou alors mettez 'rien'";
        }
    }
?>
    </div>
